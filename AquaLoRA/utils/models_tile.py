...
class SecretEncoder(nn.Module):
    def __init__(self, secret_len, base_res=32, resolution=32) -> None: # maintain the 64 res
        super().__init__()
    ...
    def forward(self, x, c):
        ...
        c = self.encode(c)
        # print(f"[Debug] watermark encoded latent shape = {c.shape}")
        ...
        # new watermark pattern
        # -------------------------------------------------
        c = c.repeat_interleave(2, dim=2)  # height: 32→64
        c = c.repeat_interleave(2, dim=3)  # width:  32→64
        # -------------------------------------------------
...
class SecretDecoder(nn.Module):
    def __init__(self, tile_num, output_size=64):
    ...
    def forward(self, x):
        # ignore the interpolate
        # x = F.interpolate(
        #     x, size=(512 // self.tile_num, 512 // self.tile_num), mode='bilinear'
        # )
