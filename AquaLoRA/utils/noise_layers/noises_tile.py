...
class CropandResize(nn.Module):
    ...
    def forward(self, noised_and_cover):
        ...
        H, W = watermarked_image.shape[-2], watermarked_image.shape[-1]
        crop_size_h = random_int(H // 2, H)
        crop_size_w = random_int(H // 2, H)
        resize_size_h = random_int(H // 2, H)
        resize_size_w = random_int(H // 2, H)

        distorted_image = T.RandomCrop(size=(crop_size_h, crop_size_w))(watermarked_image)
        distorted_image = T.Resize(size=(resize_size_h, resize_size_w),antialias=None)(distorted_image)
        distorted_image = T.Resize(size=(H, W),antialias=None)(distorted_image)
        noised_and_cover[0] = distorted_image
