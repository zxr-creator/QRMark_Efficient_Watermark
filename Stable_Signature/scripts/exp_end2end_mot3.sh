...
export CUDA_VISIBLE_DEVICES=3 # choose your GPU
...
PAPER_DECODER_PATH="${PAPER_DECODER_PATH:-models/random_grid_tile_64_48_new.pth}"
ORI_DECODER_PATH="${ORI_DECODER_PATH:-models/dec_48b.pth}"

TILE_METHOD="${TILE_METHOD:-random_grid}"
TILE_SIZE="${TILE_SIZE:-64}"
NUM_RS_THREADS="${NUM_RS_THREADS:-64}"
NUM_STREAMS="${NUM_STREAMS:-18}"
...
run_paper () {
  local bs="$1"
  local EXP_NAME="end2end_naive_streams_bs${bs}"
  ...
  append_row "paper_reserve" "QRMark_detection_paper_reserve.py" "${bs}" "${WALL}" "${PERIMG}" "${BIT}" "${WORD}" "${LOGFILE}"
}
