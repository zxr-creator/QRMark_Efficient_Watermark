...
EXPIREMENT_NAME="exp_tile_random"
# Set CUDA devices
...
for TILE_SIZE in "${TILE_SIZES[@]}"; do
  ...
  torchrun --nproc_per_node=1 --master_port=$MASTER_PORT main.py \
    ...
    --tile decode --tile_size $TILE_SIZE --tile_type random
done
EXPIREMENT_NAME="exp_tile_random_grid"
TILE_SIZES=(64)
for TILE_SIZE in "${TILE_SIZES[@]}"; do
  TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
  MASTER_PORT=$((29500 + RANDOM % (50000 - 29500)))
  torchrun --nproc_per_node=1 --master_port=$MASTER_PORT main.py \
    ...
    --scaling_w 0.3 --scale_channels False --attenuation none \
    --loss_w_type bce --loss_margin 1 \
    --tile decode --tile_size $TILE_SIZE --tile_type random_grid
done
EXPIREMENT_NAME="exp_tile_grid"
TILE_SIZES=(64)
for TILE_SIZE in "${TILE_SIZES[@]}"; do
  TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
  MASTER_PORT=$((29500 + RANDOM % (50000 - 29500)))
  torchrun --nproc_per_node=1 --master_port=$MASTER_PORT main.py \
    ...
    --scaling_w 0.3 --scale_channels False --attenuation none \
    --loss_w_type bce --loss_margin 1 \
    --tile decode --tile_size $TILE_SIZE --tile_type grid
done